...
ui <- function(id) {
  ns <- NS(id)
  cards$card_ui("Anzahl der Personen, die einer anderen Form der Diskriminierung ausgesetzt waren, gruppiert nach Alter",
...
server <- function(id) {
  moduleServer(id, function(input, output, session) {
    ...
    output$barplot <- renderPlotly({
      plotly::plot_ly(age_disc_logic$table_age_disc, x = ~category_age, y = ~Freq, color = ~discrimination, type = "bar", colors = yiord_palette,
    ...
    output$piechart <- renderPlotly({
      gg <-ggplot(age_disc_logic$table_age_disc, aes(category_age, discrimination)) +
